...
if ($sm->string_exists($identifier.'_help', $component)) {
    ...
    if ($sm->string_exists($helplink, $component)) {  // Link to further info in Moodle docs
        ...
        $completedoclink = '<div class="helpdoclink">'.$OUTPUT->doc_link($link, $linktext).'</div>';
    }
} else {
    $data->text = "<p><strong>TODO</strong>: missing help string [{$identifier}_help, $component]</p>";
}
