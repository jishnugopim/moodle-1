...
FloatingHeaders.prototype = {
    ...
    _setupFloatingAssignmentFooter: function() {
        ...
        floatingGraderFooter.setStyles({
            ...
            bottom:     0,
            height:     this.tableFooterRow.getComputedStyle(HEIGHT),
    ...
    _handleScrollEvent: function() {
        ...
        if (this.footerRow) {
            ...
            if (Y.config.win.pageYOffset + Y.config.win.innerHeight - this.pageHeaderHeight < this.footerRowPosition) {
                // The footer is off the bottom of the page.
                this.footerRow.addClass(CSS.STICKYFOOTER);
                if (Y.config.win.pageYOffset + Y.config.win.innerHeight - this.pageHeaderHeight > this.firstUserCellBottom) {
                    // The footer is above the bottom of the first user.
                    footerStyles.top = (Y.config.win.pageYOffset +
                            Y.config.win.innerHeight -
                            this.footerRow.get(OFFSETHEIGHT)) +
                            'px';
                } else {
