...
/**
 * Keep track of deprecated JS function calls.
 *
 * @var deprecations
 * @type Object
 */
...
/**
 * Announce deprecation of the specified function.
 *
 * This will cause behat tests to fail, and display a notification to developers when debugDeveloper is set.
 *
 * @param string functionName
 */
M.util.flagDeprecatedFunction = function(functionName) {
    var stackTrace;
    ...
    try {
        ...
        // Note - the stack is a prototypal function in some browsers.
        // It is not a part of the specification so a best-effort is made to find it.
        if (err.stack) {
            stackTrace = err.stack.toString();
        }
    }

    M.util.deprecations.push({
        functionName: functionName,
        stackTrace: stackTrace
    });

    if (M.cfg.developerdebug) {
        // Note: This message must go in a setTimeout because lib/javascript-static.js is loaded before requirejs.
        setTimeout(function() {
            // Note, this deliberately does not use translated strings. We try
            // not to use anything involving XHR for this.
            var message = 'The "' + functionName + '" function has been deprecated. Please refer to the upgrade notes.';
            if (stackTrace) {
                message += '<br>Stacktrace follows:<br>' + stackTrace;
            } else {
                message += '<br>No stacktrace available in this browser.';
            }
...
(function() {
    ...
    window.onerror = function() {
        M.util.exceptions.push(arguments);
