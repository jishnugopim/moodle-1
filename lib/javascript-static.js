            ...
            blockhider.prototype = {
                initializer : function(config) {
                    ...
                    if (t && (a = t.one('.block_action'))) {
                        var tooltipVisible = Y.Escape.html(config.tooltipVisible),
                            tooltipHidden  = Y.Escape.html(config.tooltipHidden),
                            hide = Y.Node.create('<img class="block-hider-hide" tabindex="0" alt="' + tooltipVisible +
                                                 '" title="' + tooltipVisible + '" />');
                        hide.setAttribute('src', this.get('iconVisible')).on('click', this.updateState, this, true);
                        hide.on('keypress', this.updateStateKey, this, true);
                        var show = Y.Node.create('<img class="block-hider-show" tabindex="0" alt="' + tooltipHidden +
                                                 '" title="' + tooltipHidden + '" />');
