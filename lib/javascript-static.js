 ...
 * @param {function} resolve The function to call when the promise is fulfilled. The value passed is the response
 * received and should contain the name, and value of the preference.
 * @param {function} reject The function to call when the promise is rejected. This value passed will contain details of
 * the error as the first parameter, and the exception dialogue as the second.
 */
M.util.get_user_preference = function(name, resolve, reject) {
    YUI().use('promise',
              'io',
              'json',
              'moodle-core-notification-ajaxexception',
              'moodle-core-notification-exception', function(Y) {
        Y.Promise(function() {
            var url = M.cfg.wwwroot + '/lib/ajax/getuserpref.php',
                cfg = {
                    data: {
                            ...
                            try {
                                ...
                                if (responsetext.error) {
                                    YUI().use('moodle-core-notification-ajaxexception', function() {
                                        reject(responsetext, new M.core.ajaxException(responsetext));
                                    });
                                } else {
                                    resolve(responsetext);
                                }
                            } catch (e) {
                                YUI().use('moodle-core-notification-exception', function() {
                                    reject(e, new M.core.exception(e));
                                });
                            }
                        }
                    }
