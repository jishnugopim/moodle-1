...
var COMPONENT = 'atto_table',
    TEMPLATE = '' +
        '<form class="{{CSS.FORM}}">' +
                ...
                '{{/if}}' +
                '{{#if appearance}}' +
                    '<div class="mdl-align">{{get_string "appearance" component}}</div>' +
                '{{/if}}' +
                '{{#if allowBorders}}' +
                    ...
                    '{{#if allowBorderSize}}' +
                        ...
                        '<br>' +
                    '{{/if}}' +
                    '{{#if allowBorderColours}}' +
                        '<label for="{{elementid}}_atto_table_bordercolour" class="sameline">' +
                        '{{get_string "bordercolour" component}}</label>' +
                        '<input id="{{elementid}}_atto_table_bordercolour" value="#FFFFFF" class="{{CSS.BORDERCOLOUR}}"' +
                        'type="text" size="8"/>' +
                        '<br>' +
                    '{{/if}}' +
                '{{/if}}' +
                '{{#if allowBackgroundColour}}' +
                    ...
                    '<input id="{{elementid}}_atto_table_backgroundcolour" value="#FFFFFF" class="{{CSS.BACKGROUNDCOLOUR}}"' +
                    'type="text" size="8"/>' +
                    '<br>' +
                '{{/if}}' +
                '{{#if allowWidth}}' +
    ...
    SELECTORS = {
        ...
        BORDERCOLOUR: '.' + CSS.BORDERCOLOUR,
        BORDERSTYLE: '.' + CSS.BORDERSTYLE,
        BACKGROUNDCOLOUR: '.' + CSS.BACKGROUNDCOLOUR,
        FORM: '.atto_form',
        WIDTH: '.' + CSS.WIDTH
    };
...
Y.namespace('M.atto_table').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
     ...
     *
     * @property _allowBorderColours
     * @type Boolean
     * @private
     */
    _allowBorderColours: null,

    /**
     * Whether or not to allow colourizing the background
     *
     * @property _allowBackgroundColour
     * @type Boolean
     * @private
     */
    _allowBackgroundColour: null,

    /**
     * Whether or not to allow setting the table width
     *
     * @property _allowWidth
     * @type Boolean
     * @private
     */
    _allowWidth: null,
    ...
    initializer: function(param) {
        ...
        this._allowBorderColours = param.allowbordercolour === '1';
        this._allowBackgroundColour = param.allowbackgroundcolour === '1';
        this._allowWidth = param.allowwidth === '1';
    ...
    _getEditDialogueContent: function() {
        ...
        if (this._allowBorders || this._allowBackgroundColour || this._allowWidth) {
            appearance = true;
        }
        ...
        this._content = Y.Node.create(template({
                ...
                allowBorders: this._allowBorders,
                allowBorderStyle: this._allowBorderStyle,
                borderStyles: this._borderStyles,
                allowBorderSize: this._allowBorderSize,
                allowBorderColours: this._allowBorderColours,
                allowBackgroundColour: this._allowBackgroundColour,
                allowWidth: this._allowWidth
            }));
    ...
    _updateTable: function(e) {
        ...
        bordercolour = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BORDERCOLOUR);
        borderstyle = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BORDERSTYLE);
        backgroundcolour = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.BACKGROUNDCOLOUR);
        width = e.currentTarget.ancestor(SELECTORS.FORM).one(SELECTORS.WIDTH);

        if (bordercolour && bordercolour.get('value')[0] === '#' &&
            (bordercolour.get('value').length == 4 || bordercolour.get('value').length == 7)) {
        ...
        if (backgroundcolour && backgroundcolour.get('value')[0] === '#' &&
            (backgroundcolour.get('value').length === 4 || backgroundcolour.get('value').length == 7)) {
    ...
    _editTable: function() {
        ...
        var node = this._getEditDialogueContent(),
            ...
            bordercolour = node.one(SELECTORS.BORDERCOLOUR),
            bordersize = node.one(SELECTORS.BORDERSIZE),
            backgroundcolour = node.one(SELECTORS.BACKGROUNDCOLOUR),
        ...
        if(table.getStyle('background-color') !== 'transparent' && this._allowBackgroundColour) {
            backgroundcolour.set('value', Y.Color.toHex(table.getStyle('background-color')));
        }

        if(this._allowBorders) {
            var bordervalue = 'none',
                bordercolourvalue = '#FFFFFF',
                borderstylevalue = 'solid',
                bordersizevalue = '1',
                interest = null;
            ...
            if(interest) {
                bordercolourvalue = interest.getStyle('border-color');
            ...
            if(bordercolour) {
                bordercolour.set('value', Y.Color.toHex(bordercolourvalue));
            }
        }
