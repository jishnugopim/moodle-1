...
var CSS = {
        ...
        INPUTLEGACYALIGN: 'atto_legacy_align'
    },
    SELECTORS = {
    ...
    TEMPLATE = '' +
            '<form class="atto_form">' +
                ...
                '<select class="{{CSS.INPUTALIGNMENT}}" id="{{elementid}}_{{CSS.INPUTALIGNMENT}}">' +
                    '{{#each alignments}}' +
                        '<option value="{{name}}">{{get_string str ../component}}</option>' +
                    '{{/each}}' +
                '</select>' +
                // Hidden input to store custom styles.
                '<input type="hidden" class="{{CSS.INPUTCUSTOMSTYLE}}"/>' +
                // Hidden input to store if the image has legacy alignment settings.
                '<input type="hidden" class="{{CSS.INPUTLEGACYALIGN}}" value="{{lagacyalign}}"/>' +
                '<br/>' +
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _applyImageProperties: function(form) {
        ...
        if (properties === false) {
            ...
            for (i in ALIGNMENTS) {
                if (ALIGNMENTS[i].isDefault === true) {
                    form.one('.' + CSS.INPUTALIGNMENT).set('value', ALIGNMENTS[i].name);
                }
            }
            return;
        }
        ...
        if (properties.align) {
            ...
            form.one('.' + CSS.INPUTLEGACYALIGN).set('value', properties.legacyalign);
        }
    ...
    _getAlignmentPropeties: function(image, properties) {
        ...
        for (i in ALIGNMENTS) {
            classname = CSS.ALIGNSETTINGS + '_' + ALIGNMENTS[i].name;
            if (image.hasClass(classname)) {
                properties.align = ALIGNMENTS[i].name;
                Y.log('Found alignment ' + ALIGNMENTS[i].name, 'debug', 'atto_image-button');
                return;
            }
        }

        // Test for a legacy alignment value.
        // First get the image style.
        style = image.getAttribute('style');
        style = style.replace(/ /g, '');
        // Then test for the legacy styles.
        for (i in ALIGNMENTS) {
            ...
            if (style.indexOf(css) !== -1) {
                ...
                if (style.indexOf(margin) !== -1) {
                    properties.align = ALIGNMENTS[i].name;
                    properties.legacyalign = true;
                    Y.log('Found legacy alignment ' + ALIGNMENTS[i].name, 'debug', 'atto_image-button');
                    return;
                }
            }
        }
        // No legacy styles have been found, so find the default.
        for (i in ALIGNMENTS) {
            if (ALIGNMENTS[i].isDefault) {
                properties.align = ALIGNMENTS[i].name;
                Y.log('Found default alignment ' + ALIGNMENTS[i].name, 'debug', 'atto_image-button');
                return;
            }
        }
    },
    ...
    _setImage: function(e) {
        var form = this._form,
            ...
            alignment = CSS.ALIGNSETTINGS + '_' + form.one('.' + CSS.INPUTALIGNMENT).get('value'),
            legacyalign = form.one('.' + CSS.INPUTLEGACYALIGN).get('value'),
        ...
        if (url !== '') {
            ...
            // If a legacy alignment was found remove it.
            if (legacyalign === 'true') {
                customstyle = this._removeLegacyAlignmnet(form.one('.' + CSS.INPUTCUSTOMSTYLE).get('value'));
            } else {
     ...
     *
     * @method _removeLegacyAlignmnet
     * @param {String} style
     * @return {String}
     * @private
     */
    _removeLegacyAlignmnet: function(style) {
        var i, margin, css;
        Y.log('Attempting to removing legacy alignment..', 'info', 'atto_image-button');
        // Normalise the css to have no space between the property and the value.
        style = style.replace(/:( )+/g, ':');
        for (i in ALIGNMENTS) {
            css = ALIGNMENTS[i].value + ':' + ALIGNMENTS[i].name + ';';
            if (style.indexOf(css) !== -1) {
                ...
                if (style.indexOf(margin) !== -1) {
                    ...
                    Y.log('...alignment found and removed.', 'info', 'atto_image-button');
                    break;
                }
            }
        }
