...
M.atto_link = M.atto_link || {
    ...
    /**
     * The toolbar button for this plugin.
     *
     * @property button
     * @type Node
     * @default null
    ...
    display_chooser : function(e, elementid) {
        ...
        var dialogue = M.atto_image.dialogue;
        if (M.atto_link.selection !== false && (!M.atto_link.selection.collapsed)) {
            if (!dialogue) {
                M.atto_link.dialogue = dialogue = new M.core.dialogue({
                    headerContent: M.util.get_string('createlink', 'atto_link'),
                    visible: false,
                    render: true,
                    modal: true,
                    close: true,
            ...
            // Set the default focusAfterHide to the button. If the dialogue was successful,
            // set_link will be called and change the focus to the editor itself.
            dialogue.setAttrs({
                        bodyContent: M.atto_link.get_form_content(elementid),
                        focusAfterHide: M.atto_link._button
                    })
                    .show(e);
        }
    },
    ...
    init : function(params) {
        ...
        M.atto_link._button = M.editor_atto.add_toolbar_button(params.elementid, 'link', iconurl, params.group, this.display_chooser, this);
    },
    ...
    set_link : function(e, elementid) {
        ...
        // Make the focus change to the contenteditable after hiding rather than the clicked button.
        M.atto_link.dialogue.set('focusAfterHide', M.editor_atto.get_editable_node(elementid))
                .hide();
