...
var COMPONENTNAME = 'atto_charmap',
    CSSBUTTON = 'atto_charmap_character',
    CSSCHARMAP = 'atto_charmap_selector',
    CHARMAP = [
        ['&nbsp;',    '&#160;',  true, 'nobreakspace'],
        ['&amp;',     '&#38;',   true, 'ampersand'],
        ['&quot;',    '&#34;',   true, 'quotationmark'],
...
Y.namespace('M.atto_charmap').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _getDialogueContent: function() {
        Y.Handlebars.registerHelper('get_charmap_string', function(identifier) {
            return M.util.get_string(identifier, 'atto_charmap');
        });
        var template = Y.Handlebars.compile(
            '<div class="{{CSS.CHARMAP}}">' +
                '{{#each CHARMAP}}' +
                    '<button class="{{../CSS.BUTTON}}" ' +
                        'aria-label="{{#get_charmap_string this.[3]}}{{/get_charmap_string}}" ' +
                        'title="{{#get_charmap_string this.[3]}}{{/get_charmap_string}}" ' +
        ...
        Y.delegate('click', this.insertChar, content, SELECTORS.BUTTON, this);
        return content;
    },
