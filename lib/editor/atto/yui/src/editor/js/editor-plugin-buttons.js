...
EditorPluginButtons.prototype = {
    ...
    addButton: function(config) {
        ...
        if (typeof config.callback === 'undefined' && typeof config.toggleTags === 'object') {
            // Note: Callback is called with the plugin context.
            // This means that `this` = the plugin defining the button.
            config.callback = function() {
                this.get('host').applyFormat(function() {
                    // Replace all of the tags witht he new classes.
                    Y.Object.each(config.toggleTags.substitutions, function(classname, tag) {
                            this.get('host').changeToCSS(tag, classname);
                        }, this);

                    Y.Array.each(config.toggleTags.applierUndos, function(cssapplier) {
                        cssapplier.undoToSelection();
                    });

                    Y.Array.each(config.toggleTags.applierToggles, function(cssapplier) {
                        cssapplier.toggleSelection();
                    });

                    // Replace CSS classes with tags.
                    Y.Object.each(config.toggleTags.substitutions, function(classname, tag) {
                            this.get('host').changeToTags(classname, tag);
                        }, this);
                }, this);
            };
        }
