YUI.add('moodle-editor_atto-manager', function (Y, NAME) {
...
/**
 * The manager for the Atto Editor.
 *
 * @module     moodle-editor_atto-manager
 * @package    editor_atto
...
var NS = YUI.namespace('M'),
    LOGNAME = 'moodle-editor_atto-manager';

NS.editor_atto = NS.editor_atto || {
    _instances: {},
    ...
    addEditorReference: function(name, reference) {
        Y.log("Registering a new atto editor: " + name, 'debug', LOGNAME);
        if (typeof this._instances[name] === 'undefined') {
            this._instances[name] = reference;
        } else {
            Y.log("An atto editor with the name '" + name + "' already exists. Unable to add.", 'warn', LOGNAME);
        }
     ...
     * "M.editor_atto.Editor"}}{{/crossLink}} for configuration options. The
     * elementid provided will be used as the name of this editor within
     * the editor Manager.
     * @return {M.editor_atto.Editor} The newly created editor instance
     */
    createEditor: function(config) {
        var instance = new Y.M.editor_atto.Editor(config);
        this.fire('editor_atto:created', {
     ...
     * @return {M.editor_atto.Editor} The requested editor instance
     */
    ...
    addMethod: function(name, fn, context) {
        if (name && fn) {
            if (typeof this[name] !== 'undefined') {
                Y.log('Overwriting existing method: ' + name, 'warn', 'moodle-editor_atto-manager');
            }
            ...
            Y.log('Unable to add method: ' + name , 'warn', 'moodle-editor_atto-manager');
        }
    }
};

Y.augment(NS.editor_atto, Y.EventTarget);

// Add methods from the Editor prototype.
NS.editor_atto.importMethod(Y.M.editor_atto.Editor.prototype, [
    'saveSelection',
    'updateFromTextArea',
    'updateOriginal',
    'disableCssStyling',
...
}, '@VERSION@', {"requires": ["base", "moodle-editor_atto-editor"]});
