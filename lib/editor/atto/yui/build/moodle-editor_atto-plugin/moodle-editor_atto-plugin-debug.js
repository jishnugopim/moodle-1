        ...
        '<div class="{{config.buttonClass}} atto_menu" ' +
            ...
            '{{#each entries}}' +
                '<div tabindex="-1" class="atto_menuentry">' +
                    '<a href="#" data-handler="{{../config.buttonClass}}_action_{{@index}}">' +
                        '{{text}}' +
                    '</a>' +
                '</div>' +
            '{{/each}}' +
...
EditorPluginButtons.prototype = {
    ...
    _showToolbarMenu: function(e, config) {
        ...
        menuDialogue.align(this.buttons[config.buttonClass], [Y.WidgetPositionAlign.TL, Y.WidgetPositionAlign.BL]);
...
}, '@VERSION@', {"requires": ["node", "base", "escape", "event", "handlebars"]});
