...
EditorCommand.prototype = {
    /**
     * Applies a callback method to editor if selection is uncollapse or waits for input to select first.
     * @method applyFormat
     * @param method A callback method which changes editor when text is selected.
     */
    applyFormat: function(callback, context) {
        // Check whether range is collapsed.
        var selection = window.rangy.getSelection();
        function handleInsert(e, callback, context, anchorNode, anchorOffset) {
            // After the keypress has inserted a character, change its level if necessary.
            Y.soon(Y.bind(function(callback, context, anchorNode, anchorOffset) {
                var selection = window.rangy.getSelection();
                ...
                // Set the start of the selection to where the it was when the button was pressed.
                ...
                this.editor.all('.editor-new-content-container').removeAttribute('class');
            }, this, callback, context, anchorNode, anchorOffset));
        }
        ...
        if (selection.isCollapsed) {
            ...
            var handle = this.editor.once('input', handleInsert, this, callback, context,
                    selection.anchorNode, selection.anchorOffset);
            // Cancel if selection changes before input.
            this.editor.onceAfter(['click', 'selectstart'], function(e, handle) {
                handle.detach();
            }, this, handle);
            return;
        }

        // The range is not collapsed; so apply method immediately.
        callback.apply(context);
