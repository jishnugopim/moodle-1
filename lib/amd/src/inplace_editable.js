...
define(['jquery', 'core/ajax', 'core/templates', 'core/notification', 'core/str', 'core/config', 'core/url'],
    ...
    $('body').on('click keypress', '[data-inplaceeditable] [data-inplaceeditablelink]', function(e) {
        ...
        var updateValueAjax = function(mainelement, value) {
        ...
        var updateValueInput = function(mainelement, value) {
            var context = $.extend({}, mainelement.data(), {
                    value: value
                });

            if (mainelement.data('defaultdisplayvalue')) {
                context.displayvalue = mainelement.data('defaultdisplayvalue');
            }
            templates.render('core/inplace_editable', context).done(function(html, js) {
                var newelement = $(html);
                templates.replaceNode(mainelement, newelement, js);
                newelement.find('[data-inplaceeditablelink]').focus();
            });
        ...
        var updateValue = function(mainelement, value) {
            if (mainelement.data('formfieldname')) {
                updateValueInput(mainelement, value);
            } else {
                updateValueAjax(mainelement, value);
            }
        };
