...
DIALOGUE = function(config) {
    ...
    config.notificationBase = Y.Node.create(
        '<div class="' + CSS.BASE + '">' +
            '<div id="' + id + '" role="dialog" aria-labelledby="' + id + '-header-text" class="' + CSS.WRAP + '">' +
            '<div class="' + CSS.HEADER + ' yui3-widget-hd"></div>' +
            '<div class="' + CSS.BODY + ' yui3-widget-bd"></div>' +
...
Y.extend(DIALOGUE, Y.Panel, {
    initializer : function(config) {
        ...
        this.once('visibleChange', this.firstVisibilityChanged, this);
    },
    firstVisibilityChanged : function(e) {
        if (this.get('draggable')) {
          Y.log("Plugging draggable");
