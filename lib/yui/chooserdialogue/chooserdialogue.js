    ...
    Y.extend(CHOOSERDIALOGUE, Y.Base, {
        ...
        setup_filter : function () {
            ...
            this.container.on('keydown', function(e) {
                ...
                if (this.searchfield_div.getStyle('display') !== 'none') {
                    this.searchfield.focus();
                    return;
                }
                ...
                optionlist = this.container.one('.alloptions');
                if (!optionlist.contains(this.searchfield_div)) {
                    optionlist.prepend(this.searchfield_div);
                }

                // Display the searchfield_div with an animation
                if (this.searchfield_div.getStyle('display') === 'none') {
                    this.searchfield_div.show();
                    anim.run();

                    // Fix the height to ensure that browsers don't change it according to max-height.
                    dialogue = this.container.one('.alloptions');
                    dialogue.setStyle('height', dialogue.getComputedStyle('height'));
                }
            ...
            this.filter = new ChooserFilter({
                ...
                // Specify that the "tags" property of each result object contains the text to filter on.
                resultTextLocator: 'tags',

                // Use a result filter to filter the results based on their tags.
                resultFilters: 'phraseMatch',

                // Create a highlight filter to highlight the node results based on the filter.
        ...
        display_chooser : function (e) {
            ...
            // Clear and hide the autocomplete searchfield
            if (this.searchfield) {
                this.searchfield_div.hide();
            ...
            // Unset the fixed height set by the searchfield.
            dialogue.setStyle('height', null);
...
'@VERSION@', {
    requires:['base', 'panel', 'moodle-core-notification', 'autocomplete-base', 'autocomplete-filters',
        'autocomplete-highlighters', 'highlight-base', 'event-key', 'event-valuechange', 'anim']
}
);
