YUI.add('moodle-core-chooserdialogue', function(Y) {
    ...
    Y.extend(CHOOSERDIALOGUE, Y.Base, {
        ...
        // The hidden radio value used to submit the disabled radio buttons value
        hiddenradiovalue : '',
        ...
        center_dialogue : function(dialogue) {
            ...
            // new max-height
            var overhead = (15 + 15 + 40 + 40);

            // Set the new max-height
            newheight = newheight - overhead;
            dialogue.setStyle('max-height', newheight + 'px');

            // We need to know the actual height of the dialogue
            var actualheight = parseInt(dialogue.getStyle('height').replace('px', ''));
            var totalheight = actualheight + overhead;
