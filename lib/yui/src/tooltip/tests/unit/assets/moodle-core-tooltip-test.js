YUI.add('moodle-core-tooltip-test', function (Y) {
    ...
    Y.Test.Runner.add(new Y.Test.Case({
        ...
        'set_header_content sets content': function() {
            this.tooltip = new M.core.tooltip();
            var fakeResponse = {};
            ...
            var header = this.tooltip.get('headerContent');
            Y.Assert.isObject(header);
            Y.Assert.areEqual(header.get('text')[0], fakeResponse.heading);
        },

        'set_body_content sets content': function() {
            this.tooltip = new M.core.tooltip();
            var fakeResponse = {};

            // Set a sample heading.
            fakeResponse.text = 'sample text';
            this.tooltip.set_body_content(fakeResponse);
            var newContent = this.tooltip.get('bodyContent');
            Y.Assert.isObject(newContent);
            Y.Assert.areEqual(fakeResponse.text, newContent.get('innerHTML'));
            Y.Assert.areEqual('alert', newContent.getAttribute('role'));
        }
    }));
    ...
    Y.Test.Runner.add(new Y.Test.Case({
        ...
        'unset footerhandler defaults correctly': function() {
            this.tooltip = new M.core.tooltip({
                footerhandler: null
            });
        ...
        'unset urlmodifier defaults correctly': function() {
            this.tooltip = new M.core.tooltip({
                urlmodifier: null
            });

            // Retrieve the urlmodifier setting
            var urlmodifier = this.tooltip.get('urlmodifier');

            // And it should exist on the M.core.tooltip object
            Y.Assert.isFunction(urlmodifier);
            Y.Assert.areEqual(this.tooltip.modify_url, urlmodifier);
        }
    ...
    Y.Test.Runner.add(new Y.Test.Case({
        name: 'Testing display of tooltip',

        tooltip: null,
        tearDown: genericTearDown,

        '_set_panel_contents works with valid content': function() {
            this.tooltip = new M.core.tooltip();
            ...
            this.tooltip._set_panel_contents(Y.JSON.stringify(response));

            // Compare the heading.
            // The heading also has a button added by the dialogue so our text is the
            // first element.
            var header = this.tooltip.get('headerContent');
            Y.Assert.isObject(header);
            Y.Assert.areEqual(header.get('text')[0], response.heading);

            // Compare the body.
            var body = this.tooltip.get('bodyContent');
            Y.Assert.isObject(body);
            Y.Assert.areEqual(response.text, body.get('innerHTML'));
            Y.Assert.areEqual('alert', body.getAttribute('role'));
        },

        '_set_panel_contents displays an appropriate error for invalid JSON': function() {
            this.tooltip = new M.core.tooltip();

            // Set the panel content.
            var errorPanel = this.tooltip._set_panel_contents('this is not json');
            Y.Assert.isObject(errorPanel);
            errorPanel.destroy();
        }
