...
class file_storage {
    ...
    protected function create_imagefile_preview(stored_file $file, $mode) {
        ...
        if ($mode === 'tinyicon') {
            $data = generate_image_thumbnail_from_string($file->get_content(), 24, 24);

        } else if ($mode === 'thumb') {
            $data = generate_image_thumbnail_from_string($file->get_content(), 90, 90);

        } else if ($mode === 'bigthumb') {
            $data = generate_image_thumbnail_from_string($file->get_content(), 250, 250);
