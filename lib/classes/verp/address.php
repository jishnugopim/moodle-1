...
/**
 * Variable Envelope Return Path address manager.
 *
 * @package    core
 * @category   verp
...
class address {
    ...
    /**
     * @const int A validation status indicating successful validation
     */
    const VALIDATION_SUCCESS = 0;

    /**
     * @const int A validation status indicating an invalid address format.
    ...
    /**
     * @const int A validation status indicating that a handler could not
     * be found for this address.
     */
    const VALIDATION_UNKNOWN_HANDLER = 2;

    /**
     * @const int A validation status indicating that an unknown user was specified.
     */
    const VALIDATION_UNKNOWN_USER = 4;

    /**
     * @const int A validation status indicating that the data key specified could not be found.
     */
    const VALIDATION_UNKNOWN_DATAKEY = 8;

    /**
     * @const int A validation status indicating that the mail processing handler was not enabled.
     */
    const VALIDATION_DISABLED_HANDLER = 16;

    /**
     * @const int A validation status indicating that the user specified was deleted or unconfirmed.
     */
    const VALIDATION_DISABLED_USER = 32;

    /**
     * @const int A validation status indicating that the datakey specified had reached it's expiration time.
     */
    const VALIDATION_EXPIRED_DATAKEY = 64;

    /**
     * @const int A validation status indicating that the hash could not be verified.
     */
    const VALIDATION_INVALID_HASH = 128;

    /**
     * @const int A validation status indicating that the originator address did not match the user on record.
     */
    const VALIDATION_ADDRESS_MISMATCH = 256;

    /**
     * The handler for the subsequent VERP commands.
     * @var \core\verp\handler
     */
    private $handler;
    ...
    /**
     * Set the handler to use for the subsequent VERP commands.
     *
    ...
    public function fetch_data_key() {
        ...
        if (!isset($this->datakey)) {
            $datakey = $DB->get_field('verp_datakeys', 'datakey', array('handler' => $this->handler->id, 'datavalue' => $this->datavalue));
            if (!$datakey) {
    ...
    protected function generate_data_key() {
        ...
        $DB->insert_record('verp_datakeys', $key);

        return $key->datakey;
    }

    /**
     * Generate an e-mail address for the VERP handler, storing a private
     * key for the data object if one was not specified.
     *
     * @param int $user The ID of the user to generated an address for.
    ...
    public function generate($userid, $userkey = null) {
        ...
        // Ensure that VERP is enabled and that there is enough information to proceed.
        if (!isset($CFG->verp_enabled) || !isset($CFG->verp_mailbox) || !isset($CFG->verp_domain)) {
            return null;
        }

        if ($userkey == null) {
            $userkey = get_user_key('verp_handler', $userid);
        }

        // Ensure that the minimum requirements are in place.
        if (!isset($this->handler)) {
            throw new coding_exception('VERP handler not specified.');
        }
        ...
        if (!isset($this->datavalue)) {
            throw new coding_exception('VERP data item has not been specified.');
        }
        ...
        $data = array(
            ...
            pack('H*', substr(md5($this->fetch_data_key() . $userkey), 0, 16)),
        );
        $subaddress = base64_encode(implode($data));

        return $CFG->verp_mailbox . '+' . $subaddress . '@' . $CFG->verp_domain;
    }

    /**
     * Process an incoming address to obtain the data stored within it.
     *
    ...
    protected function process($address) {
        ...
        if ($handler = manager::get_handler_from_id($record->handlerid)) {
            ...
            $record->data = $DB->get_record('verp_datakeys',
    ...
    protected function validate($address) {
        ...
        if (!isset($this->record->data) || !$this->record->data) {
            ...
            if (!$this->record->user) {
                ...
                $userkey = get_user_key('verp_handler', $this->record->user->id);
                $hashvalidation = substr(md5($this->record->data->datakey . $userkey), 0, 16) == $this->record->datakey;
                if (!$hashvalidation) {
                    // The address data did not check out, so the originator is deemed invalid.
                    $returnvalue += self::VALIDATION_INVALID_HASH;
                }

                if ($this->handler->validate_address) {
     ...
     */
    public function handle_message($headers, $bodyplain, $bodyhtml, $attachments) {
        if ($this->status !== self::VALIDATION_SUCCESS) {
            return $this->status;
        }

        // The state of the handler is checked during validation so it is present by this stage. No need for an extra check here.
        return $this->handler->process_message($this->record, $headers, $bodyplain, $bodyhtml, $attachments);
    }
     ...
     * @param string The encoded binary data
     */
