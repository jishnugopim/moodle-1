    ...
    public function process_message(\stdClass $record, \stdClass $data) {
        ...
        foreach ($data->attachments as $attachmenttype => $attachments) {
            foreach ($attachments as $attachment) {
                ...
                // Attempt to rename the file if it already exists.
                while ($fs->file_exists($record->contextid, $record->component, $record->filearea,
                                        $record->itemid, $record->filepath, $record->filename)) {
                    preg_match('/^.* \((\d+)\)\.[^.]*$/', $record->filename, $matches);
                    if (isset($matches[1])) {
                        $newvalue = $matches[1];
                        $newvalue++;
                        $record->filename = preg_replace('/ \((\d+)\)\.([^.]*)$/', ' (' . $newvalue . ').$2', $record->filename);
                    } else {
                        $record->filename = preg_replace('/\.([^.]*)$/', ' (1).$1', $record->filename);
                    }
                    $renamedfiles[$attachment->filename] = $record->filename;
                }
