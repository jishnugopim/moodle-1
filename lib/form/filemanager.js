...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        get_selectui: function() {
            if (typeof this.selectui === "undefined") {
                // Set up the Panel the first time it is called for only.
        ...
        update_file: function(confirmed) {
            var selectnode = this.selectnode;
            var fileinfo = this.get_selectui().fileinfo;
        ...
        setup_select_file: function() {
            ...
            selectnode.one('.fp-file-download').on('click', function(e) {
                e.preventDefault();
                if (this.get_selectui().fileinfo.type != 'folder') {
                    node = Y.Node.create('<iframe></iframe>').setStyles({
                        visibility : 'hidden',
                        width : '1px',
                        height : '1px'
                    });
                    node.set('src', this.get_selectui().fileinfo.url);
                    Y.one('body').appendChild(node);
                }
            }, this);
            selectnode.one('.fp-file-delete').on('click', function(e) {
                ...
                this.get_selectui().hide(); // TODO remove this after confirm dialog is replaced with YUI3
                this.show_confirm_dialog(dialog_options);
            }, this);
        ...
        select_file: function(node) {
            ...
            this.get_selectui().fileinfo = node;
            ...
            if (node.isref && !node.original) {
                ...
                this.request({
                    ...
                    callback: function(id, obj, args) {
                        ...
                        var scope = args.scope;
                        if (scope.get_selectui().fileinfo && node &&
                                scope.get_selectui().fileinfo.filepath == node.filepath &&
                                scope.get_selectui().fileinfo.fullname == node.fullname) {
