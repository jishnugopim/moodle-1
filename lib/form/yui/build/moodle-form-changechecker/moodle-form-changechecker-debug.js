YUI.add('moodle-form-changechecker', function (Y, NAME) {

/**
 * A tool to add confirmation when moving away from pages which have been
 * changed without saving.
 *
 * @module moodle-form-changechecker
 */
...
Y.extend(CHANGECHECKER, Y.Base, {
         ...
         *
         * @property initialvaluelisteners
         * @type {Array} The list of event listeners we have attached
         * @protected
         */
    ...
    },
    {
        ...
        ATTRS: {
            /**
             * The ID for the form to listen for changes on.
             *
             * @attribute formid
             * @type String
             * @default ''
...
// We might have multiple instances of the form change protector. These
// need to be stored in the M variable instead.
M.form_changechecker = M.form_changechecker || {};
M.form_changechecker.instances = M.form_changechecker.instances || [];

NS.init = function(config) {
    var formchangechecker = new CHANGECHECKER(config);
    M.form_changechecker.instances.push(formchangechecker);
 ...
 *
 * @method report_form_dirty_state
 */
...
// Add backwards compatability with a warning.
M.core_formchangechecker = M.core_formchangechecker || {};
M.core_formchangechecker.deprecated = function() {
    Y.log('M.core_formchangechecker has been renamed to Y.M.form_changechecker. Please update your code.', 'warn', 'moodle-form-changechecker');
};
M.core_formchangechecker.init = function() {
    M.core_formchangechecker.deprecated();
    NS.init.apply(this, arguments);
};
M.core_formchangechecker.set_form_changed = function() {
    M.core_formchangechecker.deprecated();
    NS.set_form_changed.apply(this, arguments);
};
Y.mix(M.core_formchangechecker, NS);
