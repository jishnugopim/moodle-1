...
M.form_editor = {
    ...
    setup_editor : function(ed) {
        var editor = Y.one('#' + ed.id),
            textarea = Y.Node(ed.getElement()),
            editortable = editor.ancestor('div').one('table.mceLayout'),
            thisToggleNode;
        ...
        thisToggleNode.setData('belongsto', ed.id);
    ...
    toggle_editor : function(button, editortable, newstate) {
        ...
        // TODO document what's going on here and why we're doing it
        textarea = editortable.ancestor('div').one('textarea');
        editorid = textarea.get('id');
        ...
        iframe = tinymce.DOM.get(editorid + '_ifr');
        if (iframe) {
