...
class target extends external_api {
    ...
    public static function get_target_types() {
        ...
        $types = \tool_usertours\target::get_target_types();
        $result = [];
        foreach (array_values($types) as $type) {
            $result[] = (object) [
                'type'      => $type,
                'title'     => get_string('target_' . $type, 'tool_usertours'),
            ];
        }
    ...
    public static function set_target($tourid, $typename) {
        ...
        $targettype = \tool_usertours\target::get_target_constant($params['targettype']);
        return [
            'redirectTo'    => \tool_usertours\helper::get_new_step_link($params['tourid'], $targettype)->out(false),
        ];
    }
