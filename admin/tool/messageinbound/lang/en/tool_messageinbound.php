...
$string['defaultexpiration'] = 'Default address expiration';
$string['defaultexpiration_info'] = 'When an e-mail address is generated by the handler, it can be set to automatically expire. After the address expires, it can no longer be used. It is advisable to set an expiry.';

$string['defaultexpiration_help'] = 'When a handler generates an e-mail address it can be set to automatically expire after a period of time.

When an address expires, it can no longer be used and the user must interact with Moodle directly.';
...
$string['invalid_recipient_handler'] = 'If a valid message is received but the sender cannot be authenticated, the message is stored on the e-mail server and the user is contacted using the e-mail account stored in their user profile. The user is given the chance to reply to confirm that the authenticity of the original message.

This handler processes those replies.

It is not possible to disable sender verification of this handler because the user may reply from an incorrect e-mail address if their e-mail client configuration is incorrect.';
$string['invalidrecipientdescription'] = 'The message you sent with subject "{$a->subject}" could not be authenticated.
If you were the sender of this message, and wish the message to be sent, send an reply to this message.

If you wish to avoid this message in future, please check that you are sending your message from the e-mail account listed in your Moodle profile.';
$string['invalidrecipientdescriptionhtml'] = '<p>The message you sent with subject "{$a->subject}" could not be authenticated.</p>
<p>If you were the sender of this message, and wish the message to be sent, send an reply to this message.</p>
<p><em>If you wish to avoid this message in future, please check that you are sending your message from the e-mail account listed in your Moodle profile.</em></p>';
...
$string['messageinboundgeneralconfiguration_desc'] = 'Inbound message processing allows you to receive and process e-mail within Moodle. This has applications such as sending e-mail replies to forum posts, creating new journal entries, and adding files to a user\'s private files section.';
...
$string['taskcleanup'] = 'Cleanup of unverified incoming e-mail';
$string['taskpickup'] = 'Incoming e-mail pickup';
$string['tls'] = 'TLS (TLS; started via protocol-level negotation over unencrypted channel; RECOMMENDED way of initiating secure connection)';
$string['tlsv1'] = 'TLSv1 (TLS direct version 1.x connection to server)';
$string['validateaddress'] = 'Validate sender e-mail address';
$string['validateaddress_help'] = 'When a message is received from a user, Moodle attempts to validate the message by comparing the e-mail address of the sender with the e-mail address stored in their user profile.

If the sender does not match, then the user is sent a notification to confirm that they really did send the e-mail.

If this setting is disabled, then the e-mail address of the sender is not checked at all.';
