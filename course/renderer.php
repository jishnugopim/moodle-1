...
class core_course_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    protected function course_info(coursecat_renderable $coursecatr, $course, $additionalclasses = array()) {
        global $CFG;
        ...
        $content = '';
        $classes = array_merge(array('course', 'clearfix'), $additionalclasses);
        $linkclass = 'course_link';
        if (!$course->visible) {
            $linkclass .= ' dimmed';
        }
        ...
        $coursename = html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)),
                $coursename, array('class' => $linkclass));
        if ($coursecatr->get_show_courses() >= coursecat_renderable::SHOW_COURSES_EXPANDED) {
            // wrap course name in heading tag
            $coursename = html_writer::tag('h3', $coursename, array('class' => 'course_name'));
        } else {
            $coursename = html_writer::tag('div', $coursename, array('class' => 'course_name'));
        }
        $content .= $coursename;

        // course information link and enrollment icons
        $content .= html_writer::start_tag('div', array('class' => 'course_info')); // .course_info
        ...
        if ($coursecatr->get_show_courses() >= coursecat_renderable::SHOW_COURSES_EXPANDED) {
            ...
            if ($course->has_summary()) {
                ...
                $content .= html_writer::end_tag('div'); // .course-summary
            }
            ...
            if ($coursecatr->get_show_courses() == coursecat_renderable::SHOW_COURSES_EXPANDED_WITH_CAT
                ...
                $content .= html_writer::start_tag('div', array('class' => 'course-category'));
                $content .= get_string('category').': '.
                        html_writer::link(new moodle_url('/course/category.php', array('id' => $cat->id)),
                                $cat->get_formatted_name(), array('class' => $cat->visible ? '' : 'dimmed'));
                $content .= html_writer::end_tag('div'); // .course-category
            }
            // display course contacts. See course_in_list::get_course_contacts()
            if ($course->has_course_contacts()) {
                $content .= html_writer::start_tag('ul', array('class' => 'course-contacts'));
                ...
                $content .= html_writer::end_tag('ul'); // .course-contacts
            }
        }
    ...
    protected function coursecat_courses(coursecat_renderable $coursecatr, $courses = null, $totalcount = null) {
        ...
        foreach ($courses as $course) {
            ...
            $content .= $this->course_info($coursecatr, $course, $classes);
        }
    ...
    protected function render_coursecat_renderable(coursecat_renderable $coursecatr) {
        ...
        if ($coursecatr->id && $depth) {
            ...
            if ($depth > 1) {
                ...
                $content .= html_writer::tag('h3', $categoryname, array('class' => 'category_name'));
            }
