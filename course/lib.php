...
function course_delete_module($cmid) {
    ...
    if (class_exists($moduleclass, 'update_instance')) {
        ...
        if (!$moduleclass::delete_instance($cm->instance)) {
            throw new moodle_exception('cannotdeletemoduleinstance', '', '', null,
                "Cannot delete the module $modulename (instance).");
        }
    } else {
        debugging("The old style of _delete_instance has been deprecated and will be removed." .
                "Please replace with a MODNAME_module::delete_instnace", DEBUG_DEVELOPER);
