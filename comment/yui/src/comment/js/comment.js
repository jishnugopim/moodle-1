...
Y.extend(Comment, Y.Base, {
    ...
    changePage: function(e) {
        ...
        if (!page || !page.getData('pageno')) {
            Y.log('Unable to determine which page was selected. Aborting page change.',
                    'warn', 'moodle-comment-comment');
            return;
        }
    ...
    addComment: function(e) {
        ...
        if (!commentContent) {
            Y.log('An attempt was made to save a comment, but the comment textarea could not be found.',
                    'warn', 'moodle-comment-comment');
            return;
        }
    ...
    _removeComment: function(commentRegion, payload) {
        if (!payload.commentid) {
            Y.log('The comment to remove was not specified.',
                    'warn', 'moodle-comment-comment');
            return;
        }
        ...
        if (!commentToRemove) {
            Y.log('Unable to remove comment ' + payload.commentid + ' - it was not found',
                    'warn', 'moodle-comment-comment');
            return;
        }
